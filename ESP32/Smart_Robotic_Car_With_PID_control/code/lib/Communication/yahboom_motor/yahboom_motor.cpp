...
namespace yahboom_motor
{
  ...
  static bool speedsChanged()
  {
    ...
    {
      if (data.targetSpeed[i] != data.commandedSpeed[i])
        return true;
    }
    return false;
  }
  ...
  static void parseFrame(const char *frame)
  {
    ...
    if (strncmp(frame, "MSPD:", 5) == 0)
    {
      ...
      for (uint8_t i = 0; i < 4; i++)
      {
        data.speed[i] = data.measuredSpeed[i];
      }
  ...
  static void processConfiguration()
  {
    if (data.configured)
      return;
    if (millis() - lastConfigTx < configInterval)
      return;
  ...
  static void transmitMotion()
  {
    if (!data.configured)
      return;
    if (!pendingMotionTx && !speedsChanged())
      return;
    if (millis() - lastMotionTx < motionInterval)
      return;
  ...
  static void requestUpload()
  {
    if (!data.configured)
      return;
    if (millis() - lastUploadTx < uploadInterval)
      return;
    lastUploadTx = millis();
