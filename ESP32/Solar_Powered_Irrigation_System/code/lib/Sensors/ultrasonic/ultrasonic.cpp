...
namespace ultrasonic
{
  ...
  void update()
  {
    ...
    if (s.tankEmptyDistanceCm == s.tankFullDistanceCm)
    {
      levelPercent = 0;
      return;
    }

    float percent = ((s.tankEmptyDistanceCm - distanceCm) /
                     (s.tankEmptyDistanceCm - s.tankFullDistanceCm)) *
                    100.0f;
  ...
  {
    auto &s = settings_manager::get();
    return levelPercent <= s.tankLowThreshold;
  }

  bool isTankFull()
  {
    auto &s = settings_manager::get();
    return levelPercent >= s.tankFullThreshold;
  }
}
