...
namespace web_dashboard
{
  ...
  static String relayState(bool state) { return state ? "ON" : "OFF"; }
  static String page()
  {
    String html;
    html.reserve(7000);
    ...
    html += "<header><h2>Solar Powered Egg Incubator</h2><div>IP: <span id='ip'>" + wifi_manager::getIp() + "</span> | Time: <span id='time'>" + rtc_clock::getDateTime() + "</span></div></header><main><div class='grid'>";
    ...
    html += "<div class='card'>LCD<div class='v'><span id='lcd'>" + String(lcd_screen::isAwake() ? "AWAKE" : "SLEEP") + "</span></div></div></div><h3>Controls</h3><div class='grid'>";
    ...
    html += "<div class='card'>Buzzer<br><a class='btn danger' href='/beep'>Beep</a></div><div class='card'>LCD<br><a class='btn' href='/lcd?wake=1'>Wake</a><a class='btn off' href='/lcd?wake=0'>Sleep</a></div></div></main></body></html>";
    return html;
  }
  static void sendApi()
  {
  ...
  void begin()
  {
    ...
    server.on("/api", []()
              { sendApi(); });
    server.on("/toggle", []()
              {String dev=server.arg("dev"); if(dev=="heater")heater::toggle(); if(dev=="spinner")spinner::toggle(); if(dev=="humidifier")humidifier::toggle(); server.sendHeader("Location","/"); server.send(303); });
    server.on("/beep", []()
              {buzzer::beep(100,100,2); server.sendHeader("Location","/"); server.send(303); });
    server.on("/lcd", []()
              {if(server.arg("wake")=="1")lcd_screen::wake(); else lcd_screen::sleep(); server.sendHeader("Location","/"); server.send(303); });
    server.begin();
  }
