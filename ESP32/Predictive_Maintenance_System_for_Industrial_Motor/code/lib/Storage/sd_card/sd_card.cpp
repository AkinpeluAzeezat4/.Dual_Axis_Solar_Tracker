...
namespace sd_card
{
  ...
  static const char *motorLogFileName = "/motor_log.csv";
  ...
  {
    ensureHeader(motorLogFileName, "uptime_ms,temp_c,temp_valid,current_a,current_vrms,vibration_rms_g,x_g,y_g,z_g,relay_on,fault,risk_score,health_score,level,forecast_min,backend");
    ensureHeader(analysisLogFileName, "uptime_ms,risk_score,health_score,level,worst_metric,forecast_min,temp_rate_c_per_min,current_rate_a_per_min,vibration_rate_g_per_min,recommendation");
  ...
  void update()
  {
    ...
    {
      logEvent("STATE", String("Condition changed to ") + maintenance_manager::getLevelText());
    ...
    {
      logEvent(faultNow ? "FAULT" : "RECOVERY", faultNow ? "Fault latch active. Relay tripped." : "Fault cleared.");
  ...
  void logNow()
  {
    ...
    String motorLine;
    motorLine.reserve(180);
    motorLine += String(millis());
    motorLine += ',';
    motorLine += tempText;
    motorLine += ',';
    motorLine += snap.tempValid ? "1" : "0";
    motorLine += ',';
    ...
    motorLine += ',';
    motorLine += maintenance_manager::isFault() ? "1" : "0";
    motorLine += ',';
    motorLine += String(maintenance_manager::getRiskScore(), 1);
    motorLine += ',';
    motorLine += String(maintenance_manager::getHealthScore(), 1);
    motorLine += ',';
    motorLine += maintenance_manager::getLevelText();
    motorLine += ',';
    motorLine += forecastText;
    motorLine += ',';
    motorLine += getBackendName();

    appendLine(motorLogFileName, motorLine);

    String analysisLine;
    analysisLine.reserve(220);
